...
The Web-Page Application aims to deploy web-page written Nodejs and React Frameworks on AWS Cloud Infrastructure using Ansible. Building infrastructure process is managing with control node utilizing Ansible. This infrastructure has 1 control node and 3 EC2's as worker node. These EC2's will be launched on AWS console. Web-page has 3 main components which are postgresql, nodejs, and react. Each component is serving in Docker container on EC2s dedicated for them. Postgresql is serving as Database of web-page. Nodejs controls backend part of web-side and react controls frontend side of web-page. The code was written by the Developer team and architecture will be created by the AWS & DevOps Team.

## Problem Statement

![Project_007](ansible.png)

- The company has recently ended up a project that aims to serve as web page. You and your colleagues are assigned to work on this project. The Developer team has done with code and the DevOps team is going to deploy the app in production environment using ansible.
...
- Codes written by the developers should be pulled from the repo into the control node and sent them to the EC2's from here with Ansible.
...
In the architecture, you can configure your architecture with these conditions,
  ...
  - For Nodejs worker node
    ...
    - Please make sure to correct or create `.env` file under `nodejs` folder based on PostgreSQL environmental variables
    
    - Nodejs's folder should be sent into it from control node using ansible. This file will use for docker image. You don't need any extra file for creating Nodejs image.
  ...
  - For React worker node
    ...
    - Please make sure to correct `.env` file under `react` folder based on Nodejs environmental variables 
    
    - React's folder should be sent into it from control node using ansible. This file will be used for docker image. You don't need any extra file for creating react image.
...
```text
007:ansible_project (folder)
|
|----Readme.md               # Given to the students (Definition of the project)
|----todo-app-perm (folder)  # Given to the students (Nodejs and React files)
|       1.nodejs (folder) ---> Nodejs folders and files
|       2.react (folder) ---> React folders and files
|       3.postgresql (folder)--> init.sql file 
